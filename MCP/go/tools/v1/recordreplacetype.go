...
func RecordreplacetypeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody []DNSRecordCreateType
		...
		url := fmt.Sprintf("%s/v1/domains/%s/records/%s", cfg.BaseURL, domain, type)
...
func CreateRecordreplacetypeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_v1_domains_domain_records_type",
		mcp.WithDescription("Replace all DNS Records for the specified Domain with the specified Type"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    RecordreplacetypeHandler(cfg),
	}
}
