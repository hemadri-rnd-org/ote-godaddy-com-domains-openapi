...
func RecordaddHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRecordaddTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v1_domains_domain_records",
		mcp.WithDescription("Add the specified DNS Records to the specified Domain"),
		...
		mcp.WithString("domain", mcp.Required(), mcp.Description("Domain whose DNS Records are to be augmented")),
		mcp.WithArray("items", mcp.Required(), mcp.Description("Array of items")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    RecordaddHandler(cfg),
	}
}
