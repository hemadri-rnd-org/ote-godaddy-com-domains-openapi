...
func RecordreplacetypenameHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody []DNSRecordCreateTypeName
...
func CreateRecordreplacetypenameTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_v1_domains_domain_records_type_name",
		mcp.WithDescription("Replace all DNS Records for the specified Domain with the specified Type and Name"),
		...
		mcp.WithString("domain", mcp.Required(), mcp.Description("Domain whose DNS Records are to be replaced")),
		mcp.WithString("type", mcp.Required(), mcp.Description("DNS Record Type for which DNS Records are to be replaced")),
		mcp.WithString("name", mcp.Required(), mcp.Description("DNS Record Name for which DNS Records are to be replaced")),
		mcp.WithArray("items", mcp.Required(), mcp.Description("Array of items")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    RecordreplacetypenameHandler(cfg),
	}
}
