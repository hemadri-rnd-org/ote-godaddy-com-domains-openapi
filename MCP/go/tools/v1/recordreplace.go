...
func RecordreplaceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/domains/%s/records", cfg.BaseURL, domain)
...
func CreateRecordreplaceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_v1_domains_domain_records",
		mcp.WithDescription("Replace all DNS Records for the specified Domain"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    RecordreplaceHandler(cfg),
	}
}
