...
func Post_v2_customers_customerid_domains_notifications_notificationid_acknowledgeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/customers/%s/domains/notifications/%s/acknowledge", cfg.BaseURL, customerId, notificationId)
...
func CreatePost_v2_customers_customerid_domains_notifications_notificationid_acknowledgeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_customers_customerId_domains_notifications_notificationId_acknowledge",
		mcp.WithDescription("Acknowledge a domain notification"),
		...
		mcp.WithString("notificationId", mcp.Required(), mcp.Description("The notification ID to acknowledge")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_v2_customers_customerid_domains_notifications_notificationid_acknowledgeHandler(cfg),
	}
}
