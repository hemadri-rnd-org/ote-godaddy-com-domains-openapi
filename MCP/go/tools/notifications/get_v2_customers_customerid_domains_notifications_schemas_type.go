...
func Get_v2_customers_customerid_domains_notifications_schemas_typeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/customers/%s/domains/notifications/schemas/%s", cfg.BaseURL, customerId, type)
...
func CreateGet_v2_customers_customerid_domains_notifications_schemas_typeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_customers_customerId_domains_notifications_schemas_type",
		mcp.WithDescription("Retrieve the schema for the notification data for the specified notification type"),
		...
		mcp.WithString("type", mcp.Required(), mcp.Description("The notification type whose schema should be retrieved")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_customers_customerid_domains_notifications_schemas_typeHandler(cfg),
	}
}
