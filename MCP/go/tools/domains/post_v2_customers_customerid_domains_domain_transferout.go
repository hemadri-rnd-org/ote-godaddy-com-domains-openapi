...
func Post_v2_customers_customerid_domains_domain_transferoutHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["registrar"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("registrar=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/v2/customers/%s/domains/%s/transferOut%s", cfg.BaseURL, customerId, domain, queryString)
...
func CreatePost_v2_customers_customerid_domains_domain_transferoutTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_customers_customerId_domains_domain_transferOut",
		mcp.WithDescription("Initiate transfer out to another registrar for a .uk domain."),
		...
		mcp.WithString("domain", mcp.Required(), mcp.Description("Domain to initiate the transfer out for")),
		mcp.WithString("registrar", mcp.Required(), mcp.Description("Registrar tag to push transfer to")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_v2_customers_customerid_domains_domain_transferoutHandler(cfg),
	}
}
