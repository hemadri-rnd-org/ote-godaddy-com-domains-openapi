...
func Post_v2_customers_customerid_domains_domain_renewHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DomainRenewV2
		...
		url := fmt.Sprintf("%s/v2/customers/%s/domains/%s/renew", cfg.BaseURL, customerId, domain)
...
func CreatePost_v2_customers_customerid_domains_domain_renewTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_customers_customerId_domains_domain_renew",
		mcp.WithDescription("Renew the specified Domain"),
		...
		mcp.WithString("domain", mcp.Required(), mcp.Description("Domain to be renewed")),
		mcp.WithString("expires", mcp.Required(), mcp.Description("Input parameter: Current date when this domain will expire")),
		mcp.WithNumber("period", mcp.Description("Input parameter: Number of years to extend the Domain. Must not exceed maximum for TLD. When omitted, defaults to `period` specified during original purchase")),
		mcp.WithObject("consent", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_v2_customers_customerid_domains_domain_renewHandler(cfg),
	}
}
