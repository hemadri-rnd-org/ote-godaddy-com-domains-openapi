...
func Post_v2_customers_customerid_domains_domain_redeemHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DomainRedeemV2
		...
		url := fmt.Sprintf("%s/v2/customers/%s/domains/%s/redeem", cfg.BaseURL, customerId, domain)
...
func CreatePost_v2_customers_customerid_domains_domain_redeemTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_customers_customerId_domains_domain_redeem",
		mcp.WithDescription("Purchase a restore for the given domain to bring it out of redemption"),
		...
		mcp.WithString("domain", mcp.Required(), mcp.Description("Domain to request redeem for")),
		mcp.WithObject("consent", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_v2_customers_customerid_domains_domain_redeemHandler(cfg),
	}
}
