...
func Post_v2_customers_customerid_domains_domain_transferinrestartHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/customers/%s/domains/%s/transferInRestart", cfg.BaseURL, customerId, domain)
...
func CreatePost_v2_customers_customerid_domains_domain_transferinrestartTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_customers_customerId_domains_domain_transferInRestart",
		mcp.WithDescription("Restarts transfer in request from the beginning"),
		...
		mcp.WithString("domain", mcp.Required(), mcp.Description("Domain to restart the transfer in")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_v2_customers_customerid_domains_domain_transferinrestartHandler(cfg),
	}
}
