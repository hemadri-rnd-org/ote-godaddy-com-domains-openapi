...
func Post_v2_customers_customerid_domains_domain_transferinretryHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DomainTransferAuthCode
		...
		url := fmt.Sprintf("%s/v2/customers/%s/domains/%s/transferInRetry", cfg.BaseURL, customerId, domain)
...
func CreatePost_v2_customers_customerid_domains_domain_transferinretryTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_customers_customerId_domains_domain_transferInRetry",
		mcp.WithDescription("Retries the current transfer in request with supplied Authorization code"),
		...
		mcp.WithString("domain", mcp.Required(), mcp.Description("Domain to retry the transfer in")),
		mcp.WithString("authCode", mcp.Required(), mcp.Description("Input parameter: Authorization code for transferring the Domain")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_v2_customers_customerid_domains_domain_transferinretryHandler(cfg),
	}
}
