...
func Post_v2_customers_customerid_domains_domain_transferoutacceptHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/customers/%s/domains/%s/transferOutAccept", cfg.BaseURL, customerId, domain)
...
func CreatePost_v2_customers_customerid_domains_domain_transferoutacceptTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_customers_customerId_domains_domain_transferOutAccept",
		mcp.WithDescription("Accept transfer out"),
		...
		mcp.WithString("domain", mcp.Required(), mcp.Description("Domain to accept the transfer out for")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_v2_customers_customerid_domains_domain_transferoutacceptHandler(cfg),
	}
}
