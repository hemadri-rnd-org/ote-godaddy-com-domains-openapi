...
func Post_v2_customers_customerid_domains_domain_transferinacceptHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/customers/%s/domains/%s/transferInAccept", cfg.BaseURL, customerId, domain)
...
func CreatePost_v2_customers_customerid_domains_domain_transferinacceptTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_customers_customerId_domains_domain_transferInAccept",
		mcp.WithDescription("Accepts the transfer in"),
		...
		mcp.WithString("domain", mcp.Required(), mcp.Description("Domain to accept the transfer in for")),
		mcp.WithString("authCode", mcp.Required(), mcp.Description("Input parameter: Authorization code for transferring the Domain")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_v2_customers_customerid_domains_domain_transferinacceptHandler(cfg),
	}
}
