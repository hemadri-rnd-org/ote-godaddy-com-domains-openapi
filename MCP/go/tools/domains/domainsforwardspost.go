...
func DomainsforwardspostHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDomainsforwardspostTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_customers_customerId_domains_forwards_fqdn",
		mcp.WithDescription("Create a new forwarding configuration for the given FQDN"),
		mcp.WithString("customerId", mcp.Required(), mcp.Description("The Customer identifier<br/> Note: For API Resellers, performing actions on behalf of your customers, you need to specify the Subaccount you're operating on behalf of; otherwise use your own customer id.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DomainsforwardspostHandler(cfg),
	}
}
