...
func Post_v2_customers_customerid_domains_domain_transferoutrejectHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/customers/%s/domains/%s/transferOutReject%s", cfg.BaseURL, customerId, domain, queryString)
...
func CreatePost_v2_customers_customerid_domains_domain_transferoutrejectTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v2_customers_customerId_domains_domain_transferOutReject",
		mcp.WithDescription("Reject transfer out"),
		...
		mcp.WithString("domain", mcp.Required(), mcp.Description("Domain to reject the transfer out for")),
		mcp.WithString("reason", mcp.Description("Transfer out reject reason")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_v2_customers_customerid_domains_domain_transferoutrejectHandler(cfg),
	}
}
