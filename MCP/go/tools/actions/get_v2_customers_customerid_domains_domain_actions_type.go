...
func Get_v2_customers_customerid_domains_domain_actions_typeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_v2_customers_customerid_domains_domain_actions_typeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_customers_customerId_domains_domain_actions_type",
		mcp.WithDescription("Retrieves the most recent action for the specified domain"),
		...
		mcp.WithString("domain", mcp.Required(), mcp.Description("Domain whose action is to be retrieved")),
		mcp.WithString("type", mcp.Required(), mcp.Description("The type of action to retrieve")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_customers_customerid_domains_domain_actions_typeHandler(cfg),
	}
}
