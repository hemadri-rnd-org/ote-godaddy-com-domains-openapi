...
func Delete_v2_customers_customerid_domains_domain_actions_typeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/customers/%s/domains/%s/actions/%s", cfg.BaseURL, customerId, domain, type)
...
func CreateDelete_v2_customers_customerid_domains_domain_actions_typeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v2_customers_customerId_domains_domain_actions_type",
		mcp.WithDescription("Cancel the most recent user action for the specified domain"),
		...
		mcp.WithString("domain", mcp.Required(), mcp.Description("Domain whose action is to be cancelled")),
		mcp.WithString("type", mcp.Required(), mcp.Description("The type of action to cancel")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Delete_v2_customers_customerid_domains_domain_actions_typeHandler(cfg),
	}
}
