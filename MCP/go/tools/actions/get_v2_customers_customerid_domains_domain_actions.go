...
func Get_v2_customers_customerid_domains_domain_actionsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v2/customers/%s/domains/%s/actions", cfg.BaseURL, customerId, domain)
...
func CreateGet_v2_customers_customerid_domains_domain_actionsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v2_customers_customerId_domains_domain_actions",
		mcp.WithDescription("Retrieves a list of the most recent actions for the specified domain"),
		...
		mcp.WithString("domain", mcp.Required(), mcp.Description("Domain whose actions are to be retrieved")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_v2_customers_customerid_domains_domain_actionsHandler(cfg),
	}
}
